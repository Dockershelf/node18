...
Debian packaging for Node.js 18: monolithic `nodejs` replacement packages used by the [Dockershelf node-pipeline](../node-pipeline).
...
```bash
make materialize NODE=18 DIST=trixie
make build NODE=18
```
...
| Path | Purpose |
|------|---------|
| `node/` | Upstream Node.js git submodule (`v18.x` branch) |
